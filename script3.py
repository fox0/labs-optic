...
for i, ls in enumerate((result1, result2, result3)):
    ...
    while True:
        ...
        u = (ls[-1] - x_avr) / s_x
        print('7.3. Критическое значение критерия Смирнова: u = %.3f' % u)
        print('7.4. Принимаем уровень значимости alpha = %.3f' % alpha)
        u_t = get_smirnov_k(alpha, n)
        print('7.5. Табличное значение критерия Смирнова: u_t = %.3f' % u_t)
        if u > u_t:
            print('7.6. Выражение %.3f > %.3f — истинно' % (u, u_t))
...
отвергается и в качестве рабочей принимается альтернативная. Пересчёт…''')
            ...
            print('7.6. Выражение %.3f > %.3f — ложно' % (u, u_t))
